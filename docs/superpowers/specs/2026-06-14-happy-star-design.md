...
- 身高单位厘米，最多保留一位小数；体重单位公斤，最多保留两位小数。日期为服务器本地自然日格式 `YYYY-MM-DD`。
- 身高与体重允许只记录一项，但至少一项必须有值；未测项目持久化为 `null`，不得用 `0` 或估算值填充。
- 同一孩子同一天最多一条记录；家长可新增、编辑、删除，孩子只能读取自己的记录。
- 数据按日期升序返回；身高与体重各自寻找最近两条有效值计算变化量，缺失值不参与对应曲线和变化量。
- 下次建议测量日期为最新记录日期加 3 个日历月；该日期只用于提醒，不限制提前或延期记录。
- 趋势图为两张独立SVG折线图（身高/体重），不引入图表库；缺失值不绘点，少于2个有效点时仍显示数据点和空态说明。
