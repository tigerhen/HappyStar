...
身高体重记录保存在 `data/measurements.json`，身高最多保留一位小数、体重最多保留两位小数。一次测量允许只记录身高或体重，但至少填写一项；未测项目保留为空，不补估算值。同一孩子同一天只能有一条记录：家长可在「身高体重」页新增、编辑、删除并切换孩子，孩子可在「身体」页只读查看自己的身高、体重趋势。系统按最新测量日期加 3 个日历月给出下次测量建议；提醒不限制提前或延期测量。
